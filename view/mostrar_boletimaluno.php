...
$matriculaal = $_GET['matriculaal'];
$turma1 = $_GET['nome_turma'];
?>
<input  id ="cod_turma" type="text" value= "<?=$turma?>" hidden>
<input  id ="matriculaal" type="text" value= "<?=$matriculaal?>" hidden>


<div id = "listagem" class = "listagem">
<div class="panel ">
    <div id="subcab" class="panel-heading">SAMJS - Boletim do aluno
    </div>
  ...
  <select required id="unidade" name="unidade" class="form-control ">
        ...
        <option id="MF" value="MF">Média final</option>
                    ...
                    <button id="voltarbtn" name="voltar" class="btn btn-danger botaoaluno" type="Button" onclick="window.location.href='index.php?pagina=listar_alunosturm2&cod_turma=<?=$turma?>&nome_turma=<?=$turma1?>'"><img src = 'img\retorna (1).svg' width = '20'> Voltar</button><br><br><br>
</div>
</div>
</div>
    ...
    function buscar(unidade, cod_turma, matriculaal){
        ...
        $.ajax
                ({
                    ...
                    url: 'src/boletim1.php', //pagina php responsável pela busca no bd, para a qual a função vai mandar alguns parametros
